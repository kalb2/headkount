...
export async function collectPages(
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    rows.push(...batch);
    // Connecteam pagination is complete when the API returns fewer rows than
    // requested. Some endpoints/accounts can echo the current offset instead of
    // returning the next one, so never fail solely because paging.offset did not
    // advance. The repeated-page fingerprint above still prevents infinite loops.
    if (batch.length < limit) return { rows, warnings: [] };
    const paging = json?.paging ?? json?.data?.paging;
    const next = Number(paging?.offset);
    offset =
      Number.isSafeInteger(next) && next > offset
        ? next
        : offset + batch.length;
  }
  throw new ConnecteamError(
